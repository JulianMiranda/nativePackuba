...
export const SingleTrackScreen = (props: Props) => {
  ...
  const {code} = route.params;
    ...
    useEffect(() => {
      try {
        setIsLoading(true);
        axios.post<TrackResp>(`https://www.correos.cu/wp-json/correos-api/enviosweb/`, {"codigo": code, "token": "2d2d2a9ccc", "anno": moment().format('YYYY')})
        .then((resp) => {
    ...
    return (
     ...
     <Loading />
       ...
       {data.map((item, index)=>{
        return <View key={index} style={{flexDirection: 'row',borderBottomWidth:2,borderColor: '#00CCFF', marginHorizontal: 7}}>
